...
namespace DesignPatternsInCSharp.RulesEngine.Discounts
{
    ...
    // Engine вызывает Rules, и получает результат
    public class DiscountRuleEngine
    {
        ...
        public decimal CalculateDiscountPercentageV2(Customer customer)
        {
            decimal discount = 0m;

            var rule1 = new BirthdayRule().CalculateDiscount(customer, discount);

            var rule2 = new SeniorRule().CalculateDiscount(customer, discount);
